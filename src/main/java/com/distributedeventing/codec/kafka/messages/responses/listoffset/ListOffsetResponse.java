...
public final class ListOffsetResponse extends ResponseMessage {

    /**
     * The amount of time for which the list offset request was throttled
    ...
    private final Array<ListOffsetTopicResponse> topics;
    {
        if (schema == SCHEMA_V5) {
            topics = new Array<>(new ListOffsetTopicResponse.ListOffsetTopicResponseFactory((short) 4));
        } else {
            topics = null;
        }
    }

    public static ListOffsetResponse getInstance(final short version) {
        if (version < 0 || version >= SCHEMAS.length || SCHEMAS[version] == null) {
            throw new UnsupportedVersionException(version);
        }
        return new ListOffsetResponse(createResponseHeader(version),
                                      SCHEMAS[version]);
    }
    ...
    public Array<ListOffsetTopicResponse>.ElementIterator topics() {
    ...
    private ListOffsetResponse(final ResponseHeader header,
                               final Schema schema) {
        super(header,
              schema);

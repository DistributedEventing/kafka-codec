...
public class CompactNullableString extends CompactString implements NullableValue {

    public CompactNullableString() {
        this(128, Scaler.DEFAULT_SCALER);
    }

    public CompactNullableString(final int initialCapacity,
                                 final Scaler scaler) {
    ...
    protected int sizeOfLengthInBytes() {
        ...
        return super.sizeOfLengthInBytes();
    }
    ...
    protected void validateReadLength(final int length) throws ParseException {
        if (length > Short.MAX_VALUE) {
            throw new ParseException(java.lang.String.format("Length of %s cannot exceed %d, provided length=%d",
